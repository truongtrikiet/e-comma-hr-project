<x-staff.base-layout :scrollspy="false">
    ...
    <x-custom.breadcrumb
        :breadcrumb-items="[
            ...
            __('general.menu.meeting_schedule_management.edit_meeting_schedule') => '',
        ]"
    />

    <x-form.form-layout
        :form-id="'general-settings'"
        :form-url="route('staff.meeting-schedule.update', $meetingSchedule->id)"
        :form-method="'PUT'"
        :card-title="__('general.menu.meeting_schedule_management.edit_meeting_schedule')"
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            ...
                            <x-form.form-textarea
                                ...
                                :rows="4"
                                :isRequired="false"
                                :value="$meetingSchedule->description"
                            />
                            ...
                            <x-form.form-select
                                ...
                                :selected="optional($meetingSchedule->targets->first())->target_type?->value ?? null"
                                :multiple="false"
                            ...
                            <x-form.form-input
                                ...
                                :isRequired="true"
                                :is-filter="false"
                                :value="$meetingSchedule->start_time?->format('Y-m-d\\TH:i')"
                            />
                            ...
                            <x-form.form-input
                                ...
                                :isRequired="true"
                                :is-filter="false"
                                :value="$meetingSchedule->end_time?->format('Y-m-d\\TH:i')"
                            />
    ...
    <x-slot:footerFiles>
        <script>
            ...
            window.EDIT_MEETING_TARGET_TYPE = @json(optional($meetingSchedule->targets->first())->target_type);
            window.EDIT_MEETING_TARGET_IDS = @json($meetingSchedule->targets->pluck('target_id')->toArray());
        </script>
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                function renderOptions(data, isMultiple, selectedIds = []) {
                    ...
                    data.forEach(item => {
                        ...
                        opt.value = item.id;
                        opt.text = item.name;
                        if (selectedIds && selectedIds.indexOf(item.id) !== -1) {
                            opt.selected = true;
                        }
                ...
                function handleTargetType(type, preselected = []) {
                    resetAll();
                    ...
                    if (String(type) === '1') {
                        renderOptions(window.MEETING_USERS || [], false, preselected);
                        return;
                    }

                    if (String(type) === '2') {
                        renderOptions(window.MEETING_DEPARTMENTS || [], false, preselected);
                        return;
                    }
                }
                ...
                if (window.EDIT_MEETING_TARGET_TYPE) {
                    typeSelect.value = window.EDIT_MEETING_TARGET_TYPE;
                    handleTargetType(window.EDIT_MEETING_TARGET_TYPE, window.EDIT_MEETING_TARGET_IDS || []);
