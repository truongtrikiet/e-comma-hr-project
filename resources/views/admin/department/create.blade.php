                ...
                <div class="row mt-3">
                    <div class="col-12">
                        <div class="card">
                            ...
                            <div class="card-body">
                                <div id="users-empty" class="text-muted" @if(old('user_ids')) style="display:none" @endif>
                                    {{ __('general.common.no_users_assigned') ?? 'No users assigned.' }}
                                </div>

                                <div class="table-responsive" id="users-table-wrapper" @if(!old('user_ids')) style="display:none" @endif>
                                    <input type="hidden" name="head_user_id" id="head_user_id" value="{{ old('head_user_id') ?? '' }}">
                                    <table class="table table-sm" id="users-assignment-table">
                                        ...
                                        <tbody>
                                            @if(old('user_ids'))
                                                @foreach(old('user_ids') as $oldUserId)
                                                    <tr>
                    ...
                    tbody.querySelectorAll('select[name="user_ids[]"]').forEach(select => {
                        ...
                        users.forEach(u => {
                            if (selected.includes(String(u.id)) && String(u.id) !== String(current)) {
                                return; // skip already selected elsewhere
                            }
