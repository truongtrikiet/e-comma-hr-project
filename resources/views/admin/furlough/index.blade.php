<x-base-layout :scrollspy="false">
    ...
    <x-custom.stat-box :id="'furlough-management-filter'" :custom-col="'col-lg-12'">
        <x-slot:boxTitle>
            {{ __('general.filter.title') }}
        </x-slot:boxTitle>

        @include('admin.furlough.filters.index')
    </x-custom.stat-box>
        ...
        <div>
            @can(Acl::PERMISSION_FURLOUGH_ADD)
                <x-buttons.button-link
                    ...
                    :url="route('admin.furlough.create')"
                />
            @endcan
        </div>
    </div>
            ...
            <x-slot:customScript>
                ...
                "columns": [
                    ...
                    },
                    {
                        ...
                        "orderable": false,
                        "render": function (data, type, full) {
                            let urlShow = `{{ route('admin.furlough.show', ':id') }}`.replace(':id', data);
                            let urlEdit = `{{ route('admin.furlough.edit', ':id') }}`.replace(':id', data);
                            let urlDestroy = `{{ route('admin.furlough.destroy', ':id') }}`.replace(':id', data);
