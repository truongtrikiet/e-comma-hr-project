            ...
            <x-slot:customScript>
                ...
                "columns": [
                    ...
                    },
                    {
                        ...
                        "orderable": false,
                        "render": function (data, type, full) {
                            let urlShow = `{{ route('hr.candidate-screening.show', ':id') }}`.replace(':id', data);
                            let urlEdit = `{{ route('hr.candidate-screening.edit', ':id') }}`.replace(':id', data);
                            let urlDestroy = `{{ route('hr.candidate-screening.destroy', ':id') }}`.replace(':id', data);
                            let urlSend = `{{ route('hr.candidate-screening.send-result-email', ':id') }}`.replace(':id', data);
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            submitBtn.addEventListener('click', async () => {
                ...
                try {
                    const response = await fetch(`{{ route('hr.candidate-screening.scan') }}`, {
                        method: 'POST',
            ...
            deleteConfirmBtn?.addEventListener('click', async () => {
                ...
                try {
                    ...
                    const res = await fetch("{{ route('hr.candidate-screening.delete-by-status') }}", {
                        method: 'POST',
                        headers: {
