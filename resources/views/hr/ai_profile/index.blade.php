<x-hr.base-layout :scrollspy="false">
        ...
        <div>
            @can(Acl::PERMISSION_AI_PROFILE_ADD)
                <x-buttons.button-link
                    ...
                    :url="route('hr.ai_profile.create')"
                />
            @endcan
        </div>
    </div>
            ...
            <x-slot:customScript>
                ...
                "columns": [
                    ...
                    },
                    {
                        ...
                        "orderable": false,
                        "render": function (data, type, full) {
                            <!-- let urlShow = `{{ route('hr.ai_profile.show', ':id') }}`.replace(':id', data); -->
                            let urlEdit = `{{ route('hr.ai_profile.edit', ':id') }}`.replace(':id', data);
                            let urlDestroy = `{{ route('hr.ai_profile.destroy', ':id') }}`.replace(':id', data);
    ...
    <script>
        ...
        document.getElementById('ai-call-api-btn').addEventListener('click', function() {
            ...
            const url = `{{ route('hr.ai_profile.test_api', ':id') }}`.replace(':id', id);
            const token = document.querySelector('meta[name="csrf-token"]').getAttribute('content');

            this.disabled = true;
            this.textContent = '{{ __("Calling..." ) }}';
