<x-hr.base-layout :scrollspy="false">
    ...
    <x-custom.stat-box :id="'user-management-filter'" :custom-col="'col-lg-12'">
        <x-slot:boxTitle>
            {{ __('general.common.filter') }}
        </x-slot:boxTitle>

        @include('hr.user.filters.index')
    </x-custom.stat-box>
            ...
            <x-slot:customScript>
                ...
                "columns": [
                    ...
                    },
                    {
                        ...
                        "orderable": false,
                        "render": function (data, type, full) {
                            let urlShow = `{{ route('hr.user.show', ':id') }}`.replace(':id', data);
                            let urlEdit = `{{ route('hr.user.edit', ':id') }}`.replace(':id', data);
                            let urlDestroy = `{{ route('hr.user.destroy', ':id') }}`.replace(':id', data);
